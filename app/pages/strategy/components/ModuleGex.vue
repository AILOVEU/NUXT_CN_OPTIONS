...
const gexOpt = computed(() => {
  ...
  let cum = 0; const cums = bs.map(b => { cum += b.netGex; return +(cum / 1e8).toFixed(3) })
  return Object.assign({}, BASE_OPT, {
    ...
    series: [
      { name: '认购 Gamma', type: 'bar', data: bs.map(b => +(b.gexC / 1e8).toFixed(3)), itemStyle: { color: C_UP, opacity: .75 }, barWidth: '40%' },
      { name: '认沽 Gamma', type: 'bar', data: bs.map(b => +(-b.gexP / 1e8).toFixed(3)), itemStyle: { color: C_DN, opacity: .75 }, barGap: '15%', barWidth: '40%' },
