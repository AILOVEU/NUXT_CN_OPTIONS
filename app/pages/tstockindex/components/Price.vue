<template>
  <div v-if="!props.row?._split && !props.row?._current" class="flex items-center flex-row-reverse justify-center w-[150px] mx-auto" :style="{ flexDirection: isCall ? 'row' : 'row-reverse' }">
...
const 买一 = computed(() => {
  return props.row["一手买一价"] != null ? props.row["一手买一价"] : "-";
});
const 卖一 = computed(() => {
  return props.row["一手卖一价"] != null ? props.row["一手卖一价"] : "-";
});
