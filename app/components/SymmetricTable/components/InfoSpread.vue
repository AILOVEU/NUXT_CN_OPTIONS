...
// SPREAD_DATA：多条组合，每条为 [认购, 认沽] 两项；单项（leg）结构如下：
//   leg[0] 期权名称 | leg[1] 成本价 | leg[2] 手数 | leg[3] 第一次平仓手数（第四字段） | leg[4] 第二次平仓手数（第五字段）
// 第四字段（第一次平仓手数）有值 -> 目标价格块置灰；第五字段（第二次平仓手数）有值 -> 1.5 倍目标价格块置灰
const 组合项列表 = computed(() => {
    ...
    SPREAD_DATA.forEach((group, index) => {
        ...
        if (!leg) return;
        // 提前解构成具名变量，避免后续直接使用数组下标
        const 期权名称_leg = leg[0];
        const 成本价 = leg[1];
        const 手数 = leg[2];
        const 第一次平仓手数 = leg[3];
        const 第二次平仓手数 = leg[4];
        // 该组合总成本 = 各腿（成本价 * 手数）之和
        const 总成本 = group.reduce((sum, item) => {
            const [, 单腿成本价, 单腿手数] = item;
            return sum + 单腿成本价 * 单腿手数;
        }, 0);
        // 目标价格 = 该组合总成本 / 该组合中该期权的手数
        const 目标价格 = 手数 ? 保留两位(总成本 * 1 / 手数) : 0;
        // 组合维度：任一项第二次平仓手数有值即生效，保证同一组合的认购/认沽同时变化
        const 组合有第二次平仓手数 = group.some((l) => 有值(l[4]));
        // 各块基础样式（不含字号放大）
        const 目标价格基础 = 有值(第一次平仓手数) ? 置灰样式 : 组合有第二次平仓手数 ? 白色样式 : 组合颜色(index);
        const 目标价格15倍基础 = 有值(第二次平仓手数) ? 置灰样式 : 组合有第二次平仓手数 ? 白色样式 : 组合颜色(index);
        // 字号放大：第四字段无值 -> 放大目标价格块；第四字段（第一次平仓手数）有值且第二次平仓手数无值 -> 放大 1.5 倍块；其余情况不变
        // 第二次平仓手数有值时取消放大，且按组合维度判断，保证该组合两项都不放大
        const 放大目标价格 = !有值(第一次平仓手数) && !组合有第二次平仓手数;
        const 放大目标价格15倍 = 有值(第一次平仓手数) && !有值(第二次平仓手数) && !组合有第二次平仓手数;
        // 盈亏：第四字段（第一次平仓手数）有值表示已按目标价格平仓，按 (目标价格 - 成本价) * 第一次平仓手数 计算；否则按当前一手价浮动盈亏
        const 已平仓 = 有值(第一次平仓手数);
        const 盈亏值 = 已平仓
            ? (目标价格 - 成本价) * 第一次平仓手数 + (一手价.value - 成本价) * (手数 - 第一次平仓手数)
            : (一手价.value - 成本价) * 手数;
        const 盈亏样式 = { color: 盈亏值 > 0 ? "red" : "green" };
        // 剩余手数：总手数减去已平仓部分；第一次或第二次平仓手数任一项缺失（未完全平仓）时展示
        const 已平总量 = (有值(第一次平仓手数) ? 第一次平仓手数 : 0) + (有值(第二次平仓手数) ? 第二次平仓手数 : 0);
        const 剩余手数 = 保留两位(手数 - 已平总量);
        // 剩余手数：组合任一脚有第二次平仓手数即视为完全平仓，两脚跟同时隐藏（组合维度，与涨到目标价百分比口径一致）
        const 显示剩余手数 = !组合有第二次平仓手数 && (!有值(第一次平仓手数) || !有值(第二次平仓手数));
        ...
        list.push({
            ...
            盈亏: 盈亏值,
            盈亏样式: 盈亏样式,
            // 组合有第二次平仓手数（已完全平仓）时不显示盈亏块
            显示盈亏: !组合有第二次平仓手数,
            剩余手数: 剩余手数,
            显示剩余手数: 显示剩余手数,
            // 已二次平仓（组合任一脚有第二次平仓手数）时不显示涨到目标价百分比（按组合维度判断，保证同一组合两腿一致）
            显示涨到目标百分比: !组合有第二次平仓手数 && 涨到目标百分比 != null,
            涨到目标百分比: 涨到目标百分比,
            ...
            // 第二次平仓手数有值时，该组合两项都用 2px 边框 + 2px 内边距包裹，边框取该组合的颜色
            组合边框样式: 组合有第二次平仓手数
