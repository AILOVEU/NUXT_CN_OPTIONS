import _ from "lodash";
import { getValidExerciseDate } from "~/utils/utils";
import dayjs from "dayjs";

const 最低本金贷款 = 1000; // 不消除这个贷款金了，所以接下来就留出1000用来投资，其他都提现
const 前一日贷款 = 最低本金贷款 + 10000; // 保持最低1000元贷款，还款提现部分存入出金字段
const 当日贷款 = 0;
export const 金额 = {
  第一目标: 5 * 10000,
  第二目标: 10 * 10000,
  第三目标: 20 * 10000,
  第四目标: 50 * 10000,

  // 初始入金，盈亏计算要减去它，如果现金0，贷款0，出金0，则为亏损1万
  基础金额: 10000,
  //
  场内现金: 7506,
  占用保证金: 0, // 单笔保证金50 -> 15 了
  贷款: 前一日贷款 + 当日贷款,
  出金: 65000, // 转出后不可转入，转入是贷款，新增东财逻辑
  // 👇不参与计算👇
  现转股票: 45000, //TO 10000
  国泰期货入金_股指认购: 10000, //TO -10000
  银河期货入金_股指认沽: 10000,

  华泰期货入金_期货期权: 10000,
  POLYMARKET: 686,
};
export const 建议买入价 = 380;
export const 最大建议买入时间价 = 750;
export const 最大建议买入价 = 3000;

export const fields_dict = {
  f2: "最新价", // 0.0033
  // f3: "涨跌幅", // 10.0
  // f4: "涨跌额", // 0.0003
  // f5: "成交量", // 32268
  // f6: "成交额", // 1084940.7
  // "f12": "期权代码", // 1009623
  // f13: "市场标识", //10
  f14: "期权名称", // XD50ETF购12月3117A
  f18: "昨收", // 0.0031
  f31: "买一", // 0.0031
  f32: "卖一", // 0.0033
  f108: "持仓量", // 145300
  f161: "行权价", // 3.117
  f163: "日增", // -4312
  f249: "隐波", // 15.53
  f250: "溢价率", // 3.15
  // "": '最小单位',
  // f298: "时间价值", // 0.0033
  // f299: "内在价值", // 0.0
  f301: "到期日", // 20251224
  f303: "杠杆", // 60.41
  f325: "Delta", // 0.0659
  f326: "Gamma", // 2.1658 正股变动0.01,Delta变动Gamma（正股变动1000元 , Delta 变动 Gamma*10）
  f327: "Vega", // 0.5226 隐波上涨1%,期权上涨52.26(隐波上涨1%，价格变动Vega * 100)
  f328: "Theta", // 0.0043
  f333: "正股", // XD上证50ETF
  f334: "正股价格", // 3.025
};

export const stock_index_fields_dict = {
  f2: "最新价",
  f3: "涨跌率",
  f4: "涨跌额",
  f5: "成交量",
  // # "f6": "成交额",
  // # "f12": "C期权代码",
  f13: "市场标识",
  f14: "期权名称",
  // # "f18": "昨收",
  f31: "买一",
  f32: "卖一",
  f108: "持仓量",
  f161: "行权价",
  // # "f163": "日增",
  f249: "隐波",
  // # "f250": "溢价率",
  // # "": '最小单位',
  // # "f298": "时间价值",
  // # "f299": "内在价值",
  // # "f301": "到期日",
  // # "f303": "杠杆",
  f325: "Delta",
  // # "f326": "Gamma",
  // # "f328": "Theta",
  f333: "正股",
  f334: "正股价格",
  // # "f339": "P期权代码",
  // # "f340": "P期权名称",
  // # "f341": "P最新价",
  // # "f342": "P涨跌额",
  // # "f343": "P涨跌幅",
  // # "f344": "P成交量",
  // # "f345": "P持仓量",
  // # "f346": "P隐波",
  // # "f347": "P溢价率",
};
export const OPTIONS_MAP = [
  {
    code: "510050",
    name: "50ETF",
    linkName: ["上证50ETF", "XD上证50ETF"], // 链接stock_code
    color: "#73c0de",
    showName: "50ETF🧊", // 顶部tab筛选会用到
    极简Name: "🧊",
    隐波Max: 15,
    fs: "m:10+c:510050",
  },
  {
    code: "510300",
    name: "300ETF",
    linkName: ["沪深300ETF华泰柏瑞", "XD沪深300ETF华泰柏瑞"],
    color: "#91cc75",
    showName: "300ETF🍆",
    极简Name: "🍆",
    隐波Max: 15,
    fs: "m:10+c:510300",
  },
  {
    code: "510500",
    name: "沪500ETF",
    linkName: ["中证500ETF"],
    color: "#fac858",
    showName: "沪500🦁",
    极简Name: "🦁",
    隐波Max: 22,
    fs: "m:10+c:510500",
  },
  {
    code: "159922",
    name: "深500ETF", // 中文名无关，特殊处理了
    linkName: ["深500"], // 中文名无关，特殊处理了
    color: "#fc8452",
    showName: "深500🐯",
    极简Name: "🐯",
    隐波Max: 22,
    fs: "m:12+c:159922",
  },
  {
    code: "159915",
    name: "创业板ETF",
    linkName: ["创业板ETF易方达"],
    color: "#ea7ccc",
    showName: "创业板🦑",
    极简Name: "🦑",
    隐波Max: 22,
    fs: "m:12+c:159915",
  },
  {
    code: "588000",
    name: "科50ETF",
    linkName: ["科创50ETF"],
    color: "#9a60b4",
    showName: "科50🐸",
    极简Name: "🐸",
    隐波Max: 22,
    fs: "m:10+c:588000",
  },
];
export const STOCK_INDEX_OPTIONS_MAP = [
  {
    code: "上证50",
  },
  {
    code: "沪深300",
  },
  {
    code: "中证1000",
  },
];
const 股指持仓List = [
  // 购购购购购购购购购购购购购购购购购购购购购购购
  // 300
  ["沪深300购26年7月4900", 1, 1380],
  ["沪深300购26年7月4950", 1, 700],
  ["沪深300购26年7月5000", 1, 880],
  ["沪深300购26年7月5100", 5, 720],
  ["沪深300购26年7月5200", 2, 380],
  // 50
  ["上证50购26年7月3000", 1, 1180],
  ["上证50购26年7月3050", 2, 900],
  ["上证50购26年7月3100", 4, 465],
  ["上证50购26年7月3150", 2, 170],
  // 1000
  ["中证1000购26年7月8600", 1, 1560],
  ["中证1000购26年7月8900", 1, 500],
  ["中证1000购26年7月9000", 3, 907],
  // 沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽沽
  // 300

  // 50

  // 1000
];
export const 股指持仓JSON = 股指持仓List.map((el) => ({
  期权名称: el[0],
  持仓: el[1],
  成本: el[2],
}));
export const STOCK_INDEX_DAY_MAP = {
  2607: "2026-07-17",
  2608: "2026-08-21",
  2609: "2026-09-18",
  2612: "2026-12-18",
  2703: "2027-03-19",
  2706: "2027-06-18",
};
export const STOCK_INDEX_MONTH_LIST = Object.keys(STOCK_INDEX_DAY_MAP);

export const deadline_list = getValidExerciseDate(dayjs().format("YYYY-MM-DD"));
export const deadline_color_list = ["#809b48", "#6a5286", "#40699d", "#9f413e"];
export const deadline_map = {
  "01": "1月🧳",
  "02": "2月🌂",
  "03": "3月🧵",
  "04": "4月🥁",
  "05": "5月💶",
  "06": "6月🎩",
  "07": "7月🦺",
  "08": "8月🧥",
  "09": "9月🖥️",
  10: "10月🧦",
  11: "11月🎤",
  12: "12月👑",
};
export const MONTH_ICON = {
  "01月": "🧳",
  "02月": "🌂",
  "03月": "🧵",
  "04月": "🥁",
  "05月": "💶",
  "06月": "🎩",
  "07月": "🦺",
  "08月": "🧥",
  "09月": "🖥️",
  "10月": "🧦",
  "11月": "🎤",
  "12月": "👑",
};
export const 盈亏曲线数据 = _.unionBy(
  _.reverse([
    ["20260710", 55226],
    ["20260707", 53596],
    ["20260706", 53743],
    ["20260703", 54387],
    ["20260702", 55004],
    ["20260701", 52911],
    ["20260630", 53139],
    ["20260629", 54638],
    ["20260626", 56127],
    ["20260624", 55177],
    ["20260623", 54133],
    ["20260622", 75371],
    ["20260618", 47491],
    ["20260617", 54031],
    ["20260616", 54886],
    ["20260615", 55330],
    ["20260612", 48597],
    ["20260611", 45915],
    ["20260610", 48224],
    ["20260609", 47918],
    ["20260608", 51983],
    ["20260605", 49303],
    ["20260604", 49891],
    ["20260603", 51567],
    ["20260602", 52429],
    ["20260601", 52867],
    ["20260529", 55224],
    ["20260528", 56524],
    ["20260527", 57613],
    ["20260526", 60756],
    ["20260525", 64930],
    ["20260522", 81174],
    ["20260521", 122641],
    ["20260520", 69006],
    ["20260519", 77782],
    ["20260518", 82413],
    ["20260515", 102980],
    ["20260514", 69476],
    ["20260513", 54807],
    ["20260512", 65697],
    ["20260511", 64013],
    ["20260508", 69852],
    ["20260507", 69659],
    ["20260506", 71840],
    ["20260430", 67096],
    ["20260429", 63564],
    ["20260428", 58339],
    ["20260427", 62492],
    ["20260424", 63867],
    ["20260423", 64023],
    ["20260422", 63711],
    ["20260421", 67540],
    ["20260420", 76640],
    ["20260417", 76097],
    ["20260416", 85299],
    ["20260415", 81227],
    ["20260414", 81856],
    ["20260413", 62862],
    ["20260410", 71972],
    ["20260409", 47840],
    ["20260408", 65552],
    ["20260407", 15640],
    ["20260403", 19207],
    ["20260401", 24377],
    ["20260325", 25443],
    ["20260324", -25985],
    ["20260320", -34171],
    ["20260318", -30116],
    ["20260317", -30704],
    ["20260316", -30405],
    ["20260313", -29249],
    ["20260312", -30848],
    ["20260310", -33098],
    ["20260305", -34787],
    ["20260304", -31754],
    ["20260303", -22812],
    ["20260302", -18191],
    ["20260227", -18912],
    ["20260213", -19488],
    ["20260211", -2507],
    ["20260210", 3486],
    ["20260209", 7826],
    ["20260206", -7650],
    ["20260205", -4906],
    ["20260204", 2621],
    ["20260203", -1302],
    ["20260202", -9850],
    ["20260130", 3018],
    ["20260129", 2927],
    ["20260128", -121],
    ["20260127", 739],
    ["20260126", 555],
    ["20260123", 1028],
    ["20260122", 967],
    ["20260121", 1431],
    ["20260120", 896],
    ["20260119", 460],
    ["20260116", 1766],
    ["20260115", 360],
    ["20260114", 2562],
    ["20260113", 3106],
    ["20260112", 2670],
    // 2026年国泰君安 1万重建
    // 国泰君安2025 : 21537
    // ["20260109", 21537], // 2025年收益终值
    // ["20260108", 26490],
    // ["20260107", 28488],
    // ["20260106", 32662],
    // ["20260105", 37697],
    // ["20251231", 39874],
    // ["20251230", 40978],
    // ["20251229", 39724],
    // ["20251226", 40346],
    // ["20251224", 37290],
    // ["20251223", 31926],
    // ["20251222", 32898],
    // ["20251219", 28171],
    // ["20251218", 25631],
    // ["20251217", 26978],
    // ["20251216", 19807],
    // ["20251215", 18083],
    // ["20251212", 23010],
    // ["20251208", 32596],
    // ["20251205", 24438],
    // ["20251203", 21892],
    // ["20251202", 23838],
    // ["20251201", 27178],
    // ["20251129", 26647],
    // ["20251127", 25413],
    // ["20251125", 27803],
    // ["20251124", 31145],
    // ["20251121", 34947],
    // ["20251119", 19747],
    // ["20251118", 26298],
    // ["20251117", 25889],
    // ["20251114", 28638],
    // ["20251113", 36581],
    // ["20251112", 32030],
    // ["20251110", 38904],
    // ["20251107", 39619],
    // ["20251106", 43055],
    // ["20251105", 45133],
    // ["20251103", 49306],
    // ["20251031", 53562],
    // ["20251030", 55999],
    // ["20251029", 64566],  // 2025最高
    // ["20251028", 49847],
    // ["20251027", 55480],
    // ["20251024", 46065],
    // ["20251023", 39995],
    // ["20251021", 41164],
    // ["20251020", 39923],
    // ["20251017", 42950],
    // ["20251016", 43906],
    // ["20251015", 46943],
    // ["20251014", 46176],
    // ["20251013", 45554],
    // ["20251010", 40322],
    // ["20251009", 33513],
    // 2025年初国泰君安 5万基准
    // 2024年及以前
    // 财顺期权 : 58640
    // 期财期权（新）: -851
    // 期财期权（老）: -10622
  ]),
  (el) => el[0]
).map(([name, value]) => ({
  name: dayjs(name, "YYYYMMDD").format("YYYY-MM-DD"),
  value,
}));

export const 盈亏曲线数据2025 = _.unionBy(
  _.reverse([
    // 2026年国泰君安 1万重建
    // 国泰君安2025 : 21537
    ["20260109", 21537], // 2025年收益终值
    ["20260108", 26490],
    ["20260107", 28488],
    ["20260106", 32662],
    ["20260105", 37697],
    ["20251231", 39874],
    ["20251230", 40978],
    ["20251229", 39724],
    ["20251226", 40346],
    ["20251224", 37290],
    ["20251223", 31926],
    ["20251222", 32898],
    ["20251219", 28171],
    ["20251218", 25631],
    ["20251217", 26978],
    ["20251216", 19807],
    ["20251215", 18083],
    ["20251212", 23010],
    ["20251208", 32596],
    ["20251205", 24438],
    ["20251203", 21892],
    ["20251202", 23838],
    ["20251201", 27178],
    ["20251129", 26647],
    ["20251127", 25413],
    ["20251125", 27803],
    ["20251124", 31145],
    ["20251121", 34947],
    ["20251119", 19747],
    ["20251118", 26298],
    ["20251117", 25889],
    ["20251114", 28638],
    ["20251113", 36581],
    ["20251112", 32030],
    ["20251110", 38904],
    ["20251107", 39619],
    ["20251106", 43055],
    ["20251105", 45133],
    ["20251103", 49306],
    ["20251031", 53562],
    ["20251030", 55999],
    ["20251029", 64566], // 2025最高
    ["20251028", 49847],
    ["20251027", 55480],
    ["20251024", 46065],
    ["20251023", 39995],
    ["20251021", 41164],
    ["20251020", 39923],
    ["20251017", 42950],
    ["20251016", 43906],
    ["20251015", 46943],
    ["20251014", 46176],
    ["20251013", 45554],
    ["20251010", 40322],
    ["20251009", 33513],
  ]),
  (el) => el[0]
).map(([name, value]) => ({
  name: dayjs(name, "YYYYMMDD").format("YYYY-MM-DD"),
  value,
}));

const close2024 = 58640;
const close2025 = close2024 + 盈亏曲线数据2025[盈亏曲线数据2025.length - 1].value;

export const 盈亏曲线数据年 = _.unionBy(
  _.reverse([
    {
      year: "2026",
      high: close2025 + _.max([...盈亏曲线数据.map((el) => el.value), 0]),
      low: close2025 + _.min([...盈亏曲线数据.map((el) => el.value), 0]),
      open: close2025,
      close: close2025 + 盈亏曲线数据[盈亏曲线数据.length - 1].value,
    },
    {
      year: "2025",
      high: close2024 + _.max([...盈亏曲线数据2025.map((el) => el.value), 0]),
      low: close2024 + _.min([...盈亏曲线数据2025.map((el) => el.value), 0]),
      open: close2024,
      close: close2025,
    },
    {
      year: "2024",
      high: close2024,
      low: close2024,
      open: 0,
      close: close2024,
    },
  ])
);
